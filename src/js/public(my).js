...
function setCookie(key, val, sec) {
    ...
    var date = new Date();
    if (sec) {
        date.setTime(date.getTime() - 8 * 60 * 60 * 1000 + sec * 1000);
    } else {
        date = "";
    }
...
function jsonp(url, str, cb) {
    var script = document.createElement("script");
    var callBackName = "lcb" + Math.random().toString().slice(2) + new Date().getTime();
    window[callBackName] = function (data) {
        cb(data);
    }
    script.src = url + "?" + Math.random().toString().slice(2) + "&callback=" + callBackName + "&" + str;
    script.src = script.src.slice(0, script.src.length - 1);
    document.body.appendChild(script);
}

function ajax(url, method, dataType, params, success) {
    if (dataType == "jsonp") {
        var str = "";
        jsonp(url, str, success);
    }
    if (method == "post") {
        var str = "";
        for (var i in params) {
